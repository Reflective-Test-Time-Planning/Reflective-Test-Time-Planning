...
from roboworld.envs.generator_cupboard_table_2 import generate_xml, get_color_name
...
class FrankaCupboardEnv(FrankaEnv):
    ...
    def __init__(self, cupboard_name, shape_names, shape_descriptions, model_name="main.xml",
        ...
        self.init_config = {
            ...
            'hand_init_pos': np.array((0., 0., 1.0), dtype=np.float32),
    ...
    def act_pick_up(self, obj_name):
        ...
        success = self.pickup(obj_name)
        if not success:
            return -1
        return err

    def done_pick_up(self, obj_name):
        err = self.goto(quat=[1, 0, 0, 0], pos_err_th=0.05, quat_err_th=0.01)
        return err and self.object_is_in_hand(obj_name)

    def act_put_down(self, obj_name, target_site=None, place_at_table=False):
        err = 0
        if self.object_is_in_hand(obj_name):
            if target_site is not None:
                # Put in specific compartment
                success = self.place_at_site(obj_name, target_site, place_at_table)
                if not success:
                    err = -1
            else:
                # Put on table
    ...
    def done_put_down(self, obj_name, target_site=None):
        if target_site is not None:
            ...
            return not self.object_is_in_hand(obj_name) and \
                self.pos_err(self.eef_pos, self.hand_init_pos) < 0.03 and \
                self.quat_err(self.eef_quat, self.hand_init_quat) < 0.02

    def act_txt(self, text, target_site = None, place_at_table=False):
        ...
        if "pick up" in text:
            ...
            #             except ValueError:
            #                 pass
            err = self.act_put_down(obj_name, target_site, place_at_table)
            return err
            # except ValueError:
            #     print(f"Unknown color: {color}")
            #     return -1
    ...
    def reset_object_to_init_pose(self, obj_name):
        if obj_name not in self.object_init_poses:
            print(f"[WARNING] No pose stored for {obj_name}")
            return

        # Turn off grasp weld (if any)
        try:
            weld_name = f"{obj_name}_grasp_hand"
        ...
        try:
            joint_name = f"{obj_name}_joint"
            joint_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_JOINT, joint_name)
            qpos_addr = self.model.jnt_qposadr[joint_id]
            dof_addr = self.model.jnt_dofadr[joint_id]
        except Exception as e:
            print(f"[ERROR] Joint lookup failed: {e}")
            return

        init_pose = self.object_init_poses[obj_name]

        # Normalize quaternion
        quat = np.array(init_pose["quat"])
        if np.linalg.norm(quat) < 1e-4:
            print(f"[WARNING] Invalid quaternion: {quat}")
        quat /= np.linalg.norm(quat)

        self.data.qpos[qpos_addr:qpos_addr+3] = init_pose["pos"]
        self.data.qpos[qpos_addr+3:qpos_addr+7] = quat
        self.data.qvel[dof_addr:dof_addr+6] = 0

        self.data.qvel[:] = 0  # optional: global zero
        mujoco.mj_forward(self.model, self.data)

        # Try isolating physics
        for _ in range(10):
            mujoco.mj_step(self.model, self.data)


    def force_object_on_target_pose(self, obj_name, target_pose):
        if obj_name not in self.object_init_poses:
            print(f"[WARNING] No pose stored for {obj_name}")
            return
        ...
        self.data.qpos[qpos_addr:qpos_addr+2] = target_pose[:2]
        self.data.qpos[qpos_addr+3:qpos_addr+7] = quat
        self.data.qvel[dof_addr:dof_addr+6] = 0
    ...
    def force_object_on_target_quat(self, obj_name):
        if obj_name not in self.object_init_poses:
            print(f"[WARNING] No pose stored for {obj_name}")
            return
    ...
    def release_object(self, object_name, steps_to_wait=10):
        sim = self  # for clarity

        # --- 1. Deactivate equality (weld) constraint ---
        weld_name = f"{object_name}_grasp_hand"
        weld_id = mujoco.mj_name2id(sim.model, mujoco.mjtObj.mjOBJ_EQUALITY, weld_name)
        if weld_id != -1:
            sim.data.eq_active[weld_id] = 0
        else:
            print(f"[WARNING] Weld constraint '{weld_name}' not found.")

        # --- 2. Open the gripper ---
        sim.open_gripper()

        # --- 3. Apply velocity nudge to object (free body only) ---
        body_id = mujoco.mj_name2id(sim.model, mujoco.mjtObj.mjOBJ_BODY, object_name)
        if body_id == -1:
            print(f"[WARNING] Object body '{object_name}' not found.")
        else:
            dof_addr = sim.model.body_dofadr[body_id]
            dof_num = sim.model.body_dofnum[body_id]

            if dof_num >= 6:
                # Add a slight velocity to separate the object
                sim.data.qvel[dof_addr + 2] += 0.2  # upward z-velocity
            else:
                print(f"[WARNING] '{object_name}' has no free joint; cannot apply qvel nudge.")

        # --- 4. Step simulation to allow separation ---
        for _ in range(steps_to_wait):
            mujoco.mj_step(self.model, self.data)

    def place_at_site(self, obj_name, target_site, place_at_table=False):
        ...
        success = self.goto(pos=above_target, quat=[1, 0, 0, 0])
        if not success: return False
        above_target[2] -= 0.1  # 10cm above
        if place_at_table == True:
            above_target[2] -= 0.05 
        ...
        if place_at_table:
            self.reset_object_to_init_pose(obj_name) 
        else:
            self.force_object_on_target_quat(obj_name)
        
        # Move hand away
        success = self.goto(pos=self.hand_init_pos, quat=self.hand_init_quat)
        if not success: return False
...
def transform_local_to_world(local_pos, origin_pos=[-0.15, 0.15, 0.56], origin_quat=[0.5, -0.5, 0.5, -0.5]):
...
def main():
    ...
    parser.add_argument("--seed", type=int, default=3018, help="Random seed")
    ...
    compartment_poses_min = [comp["pos_min"] for comp in info["compartments_info"]]
    compartment_poses_max = [comp["pos_max"] for comp in info["compartments_info"]]
    shape_dimensions = [shape["dimensions"] for shape in info["shapes_info"]]
    comp_dimensions = [comp["dimensions"] for comp in info["compartments_info"]]
    ...
    compartment_placed = [False for comp_dimension in comp_dimensions]
    ...
    for i in indices:
        ...
        if err == 0:
            ...
            if env.done_pick_up(first_shape):
                ...
                if compartment_placed[compartment]:
                    compartment_pose = compartment_poses_max[compartment]
                    compartment_pose[0] -= (shape_dimensions[i][0] / 2)
                    compartment_pose[0] -= 0.005
                    compartment_pose[1] += shape_dimensions[i][1] / 2 + 0.005
                    compartment_pose[2] -= (shape_dimensions[i][2] / 2)
                    compartment_pose[2] -= 0.005
                else:
                    compartment_pose = compartment_poses_min[compartment]
                    compartment_pose[0] += shape_dimensions[i][0] / 2 + 0.005
                    compartment_pose[1] += shape_dimensions[i][1] / 2 + 0.005                   
                    compartment_pose[2] += shape_dimensions[i][2] / 2 + 0.005
                
                compartment_pose = transform_local_to_world(compartment_pose)  

                compartment_placed[compartment] = True
            ...
            env.force_object_on_target_pose(first_shape, env.object_init_poses[first_shape]["pos"])

        print ("testing putting back lol")
        ...
        if env.done_pick_up(first_shape):
            ...
            env.reset_object_to_init_pose(first_shape)
            break

        err = env.act_txt(f"put down the {first_color}", env.object_init_poses[first_shape]["pos"], place_at_table=True)
