...
def main():
    """Interactive main function to test the cupboard fitting environment"""
    import argparse

    parser = argparse.ArgumentParser()
    parser.add_argument("--seed", type=int, default=3002, help="Random seed")
    ...
    compartment_names = [f"back_panel_{j}" for j in compartment_ids]
    compartment_descriptions = [info['compartment_descriptions'][comp_name] for comp_name in compartment_names]
    ...
    # Render initial state if rendering enabled
    if args.render:
        env.render()
        print("Environment window should be visible now!")
    
    # Track which shapes have been placed
    shapes_placed = set()
    
    print("\n" + "="*50)
    print("INTERACTIVE CUPBOARD SORTING TASK")
    print("="*50)
    
    # Interactive loop
    while len(shapes_placed) < len(shape_names):
        # Show current status
        print(f"\nProgress: {len(shapes_placed)}/{len(shape_names)} shapes placed")
        print(f"Remaining shapes: {len(shape_names) - len(shapes_placed)}")
        
        # Show available shapes
        print("\nAvailable shapes to pick up:")
        available_shapes = []
        for i, shape_name in enumerate(shape_names):
            if shape_name not in shapes_placed:
                available_shapes.append((i, shape_name, shape_labels[i]))
                print(f"  {len(available_shapes)}. {shape_labels[i]} ({shape_name})")
        
        # Get user choice for shape to pick up
        while True:
            try:
                choice = input(f"\nChoose a shape to pick up (1-{len(available_shapes)}) or 'q' to quit: ").strip()
                if choice.lower() == 'q':
                    print("Exiting...")
                    return
                
                choice_idx = int(choice) - 1
                if 0 <= choice_idx < len(available_shapes):
                    shape_idx, selected_shape, selected_color = available_shapes[choice_idx]
                    break
                else:
                    print(f"Please enter a number between 1 and {len(available_shapes)}")
            except ValueError:
                print("Please enter a valid number or 'q' to quit")
        
        print(f"\nAttempting to pick up {selected_color} ({selected_shape})...")
        
        # Execute pickup
        err = env.act_txt(f"pick up the {selected_color}")
        if args.render:
            env.render()  # Show pickup action
            
        if err == 0:
            print("✅ Pickup command executed successfully")
            
            # Check if pickup completed
            if env.done_pick_up(selected_shape):
                print("✅ Pickup completed successfully")
                
                # Show available compartments
                print("\nAvailable compartments:")
                for i, comp_name in enumerate(compartment_names):
                    comp_desc = compartment_descriptions[i]
                    print(f"  {i+1}. {comp_name} - {comp_desc}")
                
                # Get user choice for compartment
                while True:
                    try:
                        comp_choice = input(f"\nChoose a compartment to place the {selected_color} (1-{len(compartment_names)}) or 'skip' to place on table: ").strip()
                        
                        if comp_choice.lower() == 'skip':
                            target_compartment = None
                            target_pose = None
                            print("Placing on table...")
                            break
                        
                        comp_idx = int(comp_choice) - 1
                        if 0 <= comp_idx < len(compartment_names):
                            target_compartment = compartment_names[comp_idx]
                            target_pose = compartment_poses[comp_idx]
                            print(f"Placing in {target_compartment}...")
                            break
                        else:
                            print(f"Please enter a number between 1 and {len(compartment_names)} or 'skip'")
                    except ValueError:
                        print("Please enter a valid number or 'skip'")
                
                # Execute put down
                err = env.act_txt(f"put down the {selected_color}", target_pose)
                if args.render:
                    env.render()  # Show put down action
    ...
    # All shapes placed - final status
    print("\n" + "="*50)
    print("🎉 ALL SHAPES HAVE BEEN PLACED!")
    print("="*50)
    
    # Check overall success
    success = env.is_success(debug=True)
    print(f"\nTask success (all shapes in correct compartments): {'✅ SUCCESS' if success else '❌ PARTIAL SUCCESS'}")
    ...
    for i, shape_name in enumerate(shape_names):
        ...
        target_compartment = shape_compartment_placements[i]
        print(f"  {shape_name} ({color}): ({pos[0]:.3f}, {pos[1]:.3f}, {pos[2]:.3f}) - Target: compartment {target_compartment}")
