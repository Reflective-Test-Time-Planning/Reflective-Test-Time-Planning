...
class CupboardStructure(object):
    ...
    def __init__(self, cupboard: CupboardPartitioner, scale: float = 0.05):
    ...
    def get_body(self, pos: Optional[ArrayLike] = None, quat: Optional[ArrayLike] = None):
        ...
        # Remove back panels as requested
...
def generate_cupboard_task(max_shapes: int = 8) -> tuple:
    ...
    fitting_generator = FittingTaskGenerator(cupboard, scale=0.05)
    ...
    for comp_idx, shape in placement_options.items():
        ...
        position_shape_in_compartment(shape, compartment, 0.05)
        # shape.compartment_id = 
...
def generate_xml(seed: int) -> tuple[XmlMaker, dict]:
    ...
    cupboard_structure = CupboardStructure(cupboard, scale=0.05)
    ...
    for i, compartment in enumerate(cupboard.compartments):
        if i < len(shapes):  # Only add sites for shapes we have
            target_site = create_element("site", parent=cupboard_body, attributes={
                ...
                "pos": [
                    (compartment.min_x + compartment.max_x) / 2 * 0.05,
                    (compartment.min_y + compartment.max_y) / 2 * 0.05,
                    (compartment.min_z + compartment.max_z) / 2 * 0.05 + 0.02
                ],
                "class": "invisible_site"
            })
        ...
        "compartments_info": [
            {
                ...
                "physical_bbox": {
                    "min_x": comp.min_x * 0.05, "min_y": comp.min_y * 0.05, "min_z": comp.min_z * 0.05,
                    "max_x": comp.max_x * 0.05, "max_y": comp.max_y * 0.05, "max_z": comp.max_z * 0.05
                },
                "color": comp.color
            }
