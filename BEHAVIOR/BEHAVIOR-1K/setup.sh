...
if [ "$NEW_ENV" = true ]; then
    echo "Creating conda environment 'behavior-env'..."
    ...
    if conda env list | grep -q "^behavior-env "; then
        echo ""
        echo "ERROR: Conda environment 'behavior-env' already exists!"
        echo ""
    ...
    conda create -n behavior-env python=3.10 -c conda-forge -y
    conda activate behavior-env

    [[ "$CONDA_DEFAULT_ENV" != "behavior-env" ]] && { echo "ERROR: Failed to activate environment"; exit 1; }
...
if [ "$NEW_ENV" = true ]; then echo "✓ Created conda environment 'behavior-env'"; fi
...
echo ""
if [ "$NEW_ENV" = true ]; then echo "To activate: conda activate behavior-env"; fi