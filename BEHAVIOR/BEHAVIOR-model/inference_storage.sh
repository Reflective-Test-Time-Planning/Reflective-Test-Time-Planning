...
# Bash script to run storage task inference on multiple scene configs
# Usage: ./run_storage_batch.sh [TYPE]
...
SCENE_CONFIG_DIR="storage_data_sample"
OUTPUT_DIR="storage_results_final_${EXPERIMENT_TYPE}"
LOG_DIR="storage_logs_final_${EXPERIMENT_TYPE}"
...
# Get all .json files from the directory and shuffle them
echo "Loading scene files from $SCENE_CONFIG_DIR..."
SCENE_FILES=$(find "$SCENE_CONFIG_DIR" -name "*.json" -type f -exec basename {} .json \; | shuf)
...
while IFS= read -r scene_file; do
    ...
    # Construct full path to scene config
    SCENE_CONFIG_PATH="$SCENE_CONFIG_DIR/$scene_file.json"
    
    # Check if file exists
    if [ ! -f "$SCENE_CONFIG_PATH" ]; then
        echo "Warning: Scene config not found: $SCENE_CONFIG_PATH"
        echo "Skipping..."
        FAILED=$((FAILED + 1))
        continue
    fi
    ...
    python inference_storage.py \
        --model-path "$MODEL_PATH" \
        --scene-config "$SCENE_CONFIG_PATH" \
        $LOAD_4BIT \
    ...
    rm -rf room_pointclouds_storage/*
