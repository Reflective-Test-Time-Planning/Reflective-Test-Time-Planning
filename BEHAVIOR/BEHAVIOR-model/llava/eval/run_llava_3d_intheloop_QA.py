...
from validate_traj import validate_trajectory
...
def eval_model(args):
    ...
    # with open('/local1/whu/llava_3d_files/llava-3d_data_Feb26_v6_medium_train136_lastfix_annotation.json', 'r') as f: 
    ...
    with open('/local1/whu/llava_3d_files/llava-3d_data_Feb26_v6_medium_train136_lastfix_QA_annotation.json', 'r') as f:
    # ann_path = '/local1/whu/llava_3d_files/Feb26_v6_medium_train136_lastfix_QA_on0302'
    # os.listdir(ann_path)
        annotations = json.load(f)
    
    all_predictions = []
    for ann in tqdm(annotations):
        # with open(os.path.join(ann_path, ann), 'r') as f:
        #     ann = json.load(f)

        ann_QAs = ann['answer']
        ann_Qs = []
        ann_As = []
        QA_num = len(ann_QAs) // 2
        for i in range(1, QA_num+1, 1):
            ann_Qs.append(ann_QAs['question_' + str(i)])
            ann_As.append(ann_QAs['answer_' + str(i)])

        all_room_objects = ann['train_data']['room_objects']
        room_order = ann['train_data']['answer']['Room Order']
        gt_trajectory = ann['train_data']['answer']['Trajectory']
        obj_result, evaluation_log, valid = validate_trajectory(room_order, gt_trajectory,all_room_objects,break_on_error=True)
        ...
        system_message = ann['train_data']['system_message'].replace('<video>', '<image>')
        video_paths = ann['train_data']['training_data']['video']
        video_paths = ['/local1/whu/data/hm3d/semantic_room_data/original_2d_gt_seg_multiple_x_0226_v6_medium_500_lastfix_addobjects/'+v for v in video_paths]
        training_conv = ann['train_data']['training_data']['conversations']
    ...
    # ["/local1/whu/data/hm3d/semantic_room_data/original_2d_gt_seg_multiple_x_0106_addobjects/hm3d/00031-Wo6kuutE9i7_0", "/local1/whu/data/hm3d/semantic_room_data/original_2d_gt_seg_multiple_x_0106_addobjects/hm3d/00031-Wo6kuutE9i7_3"]' \
        ...
        for train_conv in training_conv:
            if train_conv['from'] == 'human':
                ...
                conv.append_message(conv.roles[1], train_conv['value'])
            else: 
                print('unknown role \n\n\n')  

        all_QA_predictions = {}
        all_QA_predictions['question'] = ann_Qs
        all_QA_predictions['answer'] = ann_As
        all_QA_predictions['prediction'] = []
            # outputs ='place holder'
            # last_out.append(outputs)
        for q,a in zip(ann_Qs, ann_As):
            QA_conv = deepcopy(conv)
            QA_conv.append_message(QA_conv.roles[0], q)
            prompt = QA_conv.get_prompt()
            print("prompt", prompt)
            input_ids = (
            ...
            all_QA_predictions['prediction'].append(outputs)
        ...
        # outputs = outputs.strip()
        # print(outputs)
        ann['prediction'] = all_QA_predictions
        all_predictions.append(ann)

    outputpath = '/home/whu/vl_research/MultiPLY/evaluation/evaluation_output'
    outputfile = '3d_data_Feb26_train136_multirooms_annotation_allevaled_0.4voxelep12_medium_v6_QA_trainedModel_test01on0310' + args.model_path.split('/')[-1] + '.json'
