...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_rgbd_videos(self, images, world_points, depths, poses, intrinsics, lengths=None):
        ...
        # print (image_features.shape, world_points.shape)
        video_features, batch_offset = self.get_model().get_video_tower()(image_features, world_points, depths, poses, intrinsics, lengths=lengths)  # (B, 1024, C)
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        images, world_points, depths, poses, intrinsics, lengths, clicks=None, image_sizes=None
    ):
        ...
        flattened_images_count = [len(x) for x in images]
        flatten_images = [x for xs in images for x in xs]
        # flatten_depths = [x for xs in depths for x in xs]
        # depths = torch.stack(flatten_depths)  
        # flatten_poses = [x for xs in poses for x in xs]
        # poses = torch.stack(flatten_poses)  
        # flatten_intrinsics = [x for xs in intrinsics for x in xs]
        # intrinsics = torch.stack(flatten_intrinsics)  

        videos_minibatch= torch.stack(flatten_images) # [6, 20, 3, 336, 336])
        # print(f"Videos Minibatch: {videos_minibatch.shape}\n") # (2, 20, 3, 336, 336)
        # print('len videos_minibatch: {}, len depths: {}, len poses: {}, len intrinsics: {}'.format(len(videos_minibatch), len(depths), len(poses), len(intrinsics)))
        # if len(videos_minibatch) > 0:
        #     assert len(videos_minibatch) == len(depths) == len(poses) == len(intrinsics)

        image_features = [None] * len(videos_minibatch) #(len(image_idx) + len(video_idx))  # len(B)
        ...
        if getattr(videos_minibatch, 'ndim', 0) == 5:  # batch consists of videos, [mini_b_2, v, 3, h, w]
            video_features_minibatch, batch_offset = self.encode_rgbd_videos(videos_minibatch, world_points, depths, poses, intrinsics, lengths=lengths)
            # print('batch_offset:', batch_offset)
                    #             0, 160], device='cuda:6', dtype=torch.int32)
                    # tensor([ 50, 130], device='cuda:4', dtype=torch.int32)
                    # tensor([ 80, 130], device='cuda:2', dtype=torch.int32)
                    # tensor([ 50, 130], device='cuda:1', dtype=torch.int32)
                    # tensor([ 84, 164], device='cuda:0', dtype=torch.int32)
        ...
        if clicks is None:  # 1. no video data 2. the video data does not contain the click case in the batch
            pseudo_clicks = images[0][0].new_zeros((0, 3)) #images[0].new_zeros((0, 3))
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            # pUrint(f"Num Images: {num_images}\n") # 0
            # ------------------------------------------------------
            ...
            # print('cur_new_input_embeds:', cur_new_input_embeds.shape) # torch.Size([2, 1024])
            # print('prompt_features:', prompt_features[0:0].shape) # torch.Size([0, 1024])
            # if num_prompts == 0:
            #     cur_new_input_embeds = torch.cat([cur_new_input_embeds, prompt_features[0:0]], dim=0)
            ############ WENBO HARDCODE: REMOVE prompt_features
