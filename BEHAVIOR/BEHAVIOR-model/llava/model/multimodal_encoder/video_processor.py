...
class RGBDVideoProcessor(ProcessorMixin):
    ...
    """
    def __init__(self, vision_tower_name, num_frames=12, tokenizer=None, **kwargs): #num_frames=24
        ...
        # with open('./playground/data/annotations/embodiedscan_infos_full.json', 'r') as file: original_2d_gt_seg_multiple_x_0211_train_addobjects
        # with open('/local1/whu/data/hm3d/semantic_room_data/original_2d_gt_seg/00031-Wo6kuutE9i7_0/scan_infos_full.json', 'r') as file:
        # with open(os.path.join('/local1/whu/data/hm3d/semantic_room_data/original_2d_gt_seg_one_grid_0106_addobjects', '00031-Wo6kuutE9i7_one_grid_scan_infos_full.json'), 'r') as file:
        # with open(os.path.join('/local1/whu/data/hm3d/semantic_room_data/original_2d_gt_seg_multiple_x_0227_train_original_room_all', '00031-Wo6kuutE9i7_one_grid_scan_infos_full.json'), 'r') as file:
        # with open(os.path.join('/local1/whu/data/hm3d/semantic_room_data/original_2d_gt_seg_multiple_x_0226_v6_medium_180scenes_18k_gemini20flash_v7', '00031-Wo6kuutE9i7_one_grid_scan_infos_full.json'), 'r') as file:
        # with open(os.path.join('/local1/whu/data/hm3d/semantic_room_data/original_2d_gt_seg_multiple_x_0226_v6_simple_180scenes_18x5k_gemini20flash_v7', '00031-Wo6kuutE9i7_one_grid_scan_infos_full.json'), 'r') as file:
        # with open(os.path.join('/local1/whu/data/hm3d/semantic_room_data/original_2d_gt_seg_multiple_x_0226_v6_0415_simple_180_scenes_18kx10_gemini20flash_v7', '00031-Wo6kuutE9i7_one_grid_scan_infos_full.json'), 'r') as file:
        #with open(os.path.join('/local1/whu/data/hm3d/semantic_room_data/original_2d_gt_seg_multiple_x_0226_v6_0415_medium_180_scenes_18kx10_gemini20flash_v7', '00031-Wo6kuutE9i7_one_grid_scan_infos_full.json'), 'r') as file:
        # with open(os.path.join('/local1/whu/data/hm3d/semantic_room_data/original_2d_gt_seg_multiple_x_0226_v6_0415_hard_180_scenes_18kx15_gemini20flash_v7', '00031-Wo6kuutE9i7_one_grid_scan_infos_full.json'), 'r') as file:
    ...
    def extract_embodiedscan_video(self, video):
        ...
        ########### video name:
        video_name = 'hm3d/' + video_name.split('/')[-1] + '/'  # 00031-Wo6kuutE9i7_0
        ...
        for frame in sample_frames:
            ...
            # image = image.replace('_data/hm3d', '_data/original_2d_gt_seg_multiple_x_0226_v6_medium_180scenes_18k_gemini20flash_v7')
            # image = image.replace('_data/hm3d', '_data/original_2d_gt_seg_multiple_x_0227_train_original_room_all')
            # image = image.replace('_data/hm3d', '_data/original_2d_gt_seg_multiple_x_0226_v6_0415_simple_180_scenes_18kx10_gemini20flash_v7')
            image = image.replace('_data/hm3d', '_data/original_2d_gt_seg_multiple_x_0226_v6_0415_medium_180_scenes_18kx10_gemini20flash_v7')
            # image = image.replace('_data/hm3d', '_data/original_2d_gt_seg_multiple_x_0226_v6_0415_hard_180_scenes_18kx15_gemini20flash_v7')

            
            # image = image.replace('_data/hm3d', '_data/original_2d_gt_seg_multiple_x_0226_v6_simple_180scenes_18x5k_gemini20flash_v7')

            # image = image.replace('_data/hm3d', '_data/original_2d_gt_seg_multiple_x_0226_v6_medium_500_lastfix_addobjects') #original_2d_gt_seg_multiple_x_0217_5room_train_addobjects') ##original_2d_gt_seg_multiple_x_0217_5room_train_addobjects') # for hm3d ####HARD CODE 
            substring = "hm3d/"
            split_parts = image.split(substring, 2)  # Split into at most 3 parts
            image = substring.join(split_parts[:2]) + split_parts[2] 
            images.append(image)
            if 'scannet' in frame:
                ...
                # depth = os.path.join(video_folder, video_info[frame]['depth'])
                # depth = depth.replace('_data/hm3d', '_data/original_2d_gt_seg_one_grid_12_26_fix') # for hm3d ####HARD CODE 
                # break 
                pass
            else:
                raise NotImplementedError
            ...
            # depths.append(depth)
            # poses.append(pose)
        ppath = '/'.join(image.split('/')[:-1])
        point_path = os.path.join(ppath, 'points.npy') 
        world_points = np.load(point_path)
        ...
        #     sampled_video_info['depth_intrinsic_file'] = depth_intrinsic_file
            intrinsic_file  = None
            # intrinsic_file = np.stack(intrinsics, axis=0)
            # print('intrinsic_file: ', intrinsic_file.shape)
            sampled_video_info['depth_intrinsic_file'] = None
        ...
        sampled_video_info['world_points'] = world_points   
        return sampled_video_info

    def extract_embodiedscan_behavior(self, video):
        # video is the full path for the video
        video_path = video

        images = [os.path.join(video_path, f) for f in os.listdir(video_path) if f.endswith('.png')]
        images.sort()

        point_path = os.path.join(video_path, 'points.npy') 
        world_points = np.load(point_path)
        sampled_video_info = dict()
        ...
        # axis_align_matrix_file = np.array(video_info['axis_align_matrix'])  # 4x4 array
        sampled_video_info['sample_image_files'] = images
        sampled_video_info['sample_depth_image_files'] = None
        sampled_video_info['sample_pose_files'] = None
        sampled_video_info['intrinsic_file'] = intrinsic_file
        # sampled_video_info['axis_align_matrix_file'] = axis_align_matrix_file
        sampled_video_info['dataset'] = "hm3d"
        sampled_video_info['sample_frame_num'] = len(images)
        sampled_video_info['world_points'] = world_points   
        return sampled_video_info
    ...
    def preprocess(self, 
        ...
        if isinstance(video, list):   # list of video frames only could be embodiedscan data
            ...
            print('using openscan data')
        else:
            # print('using embodiedscan data')
            video_info = self.extract_embodiedscan_behavior(video)
        ...
        if 'depth_intrinsic_file' in video_info and video_info['depth_intrinsic_file'] is not None:
        ...
        for id, image_file in enumerate(video_info['sample_image_files']):
            ...
            # poses.append(pose)
        world_points = torch.from_numpy(video_info['world_points']) #.float()  # (V*H*W, 3)
        ...
        # print('images: ', video_dict['images'].shape)
        video_dict['world_points'] = world_points
