...
class RGBDVideoTower(nn.Module):
    ...
    def forward(self, features, world_points, depths, poses, intrinsics, lengths=None):
        ...
        for offset in range(1, bs+1, 1):
            ...
            mem1.append(video_features[0])
            mem2.append(video_features[1])
        ...
        for mem_1, mem_2, working_mem in zip(mem1, mem2, working_mem_bs):
            new_pooled_video_features.append(torch.cat((mem_1, mem_2, working_mem), dim=0))
            working_mem_offset += len(mem_1) 
            new_batch_offset.append(working_mem_offset)
            working_mem_offset += len(mem_2) 
            new_batch_offset.append(working_mem_offset)
