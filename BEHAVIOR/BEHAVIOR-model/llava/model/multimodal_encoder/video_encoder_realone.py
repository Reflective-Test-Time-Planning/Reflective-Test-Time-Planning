...
class RGBDVideoTower(nn.Module):
    def __init__(self, vision_tower, video_tower, args, delay_load=False):
        ...
        self.query = nn.Parameter(torch.zeros(1024, 1024))
    ...
    def forward(self, features, world_points, depths, poses, intrinsics, lengths=None):
        ...
        ############ memory QKV ####
        per_sample_len = batch_offset.shape[0] // bs
        last_offset = 0
        batch_q = []
        batch_k = []
        batch_v = []
        ...
        for offset in range(1, bs+1, 1):
            ...
            cur_q = self.mem_q(self.query)   #video_features[-2]) #### -2 

            proj_feats_k = []
            proj_feats_v = []
            
            if len(video_features) > 1:
                episodic_features = video_features[:-1]
            else:
                episodic_features = video_features

            for room_id, feat in enumerate(episodic_features):
                ...
                proj_feats_k.append(proj_k + pos_emb_k)
                proj_feats_v.append(proj_v + pos_emb_v)
            ...
            # random_longterm_mem.append(video_features[-2])
            
            last_offset += per_sample_len
        ...
        #print("mem_attn input shapes:")
        #print("q:", batch_q.shape)
        #print("k:", batch_k.shape)
        #print("v:", batch_v.shape)
