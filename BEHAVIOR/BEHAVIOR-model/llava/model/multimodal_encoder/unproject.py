...
def unproject_old(intrinsics, poses, depths):
    """
    Inputs:
...
def unproject(intrinsics, poses, depths, locs):
    """
    Inputs:
        intrinsics: B X V X 3 X 3
        poses: B X V X 4 X 4 (torch.tensor)
    ...
    Outputs:
        world_coords: B X V X H X W X 3
    """
    # Assuming _min, _max, camera2agent, and loc are defined
    # _min, _max = 0, 100
    # depths = torch.clamp(depths, min=_min, max=_max)
    # valid_mask = (depths > _min) & (depths < _max)

    B, V, H, W = depths.shape
    fx, fy, px, py = intrinsics[..., 0, 0][..., None], intrinsics[..., 1, 1][..., None], intrinsics[..., 0, 2][..., None], intrinsics[..., 1, 2][..., None]

    y = torch.arange(H - 1, -1, -1).to(depths.device)  # Reverse y-axis
    x = torch.arange(W).to(depths.device)
    y, x = torch.meshgrid(y, x)
    ...
    x = (x - px) * z / fx
    y = (y - py) * z / fy
    cam_coords = torch.stack([x, y, z, torch.ones_like(x)], -1)  # (B, V, H*W, 4)

    world_coords = (poses @ cam_coords.permute(0, 1, 3, 2)).permute(0, 1, 3, 2)
    world_coords = world_coords[..., :3] / world_coords[..., 3][..., None]

    # Reverse z-axis
    world_coords[..., 2] = loc[..., 2][..., None, None] - world_coords[..., 2]
...
def backprojector_dataloader(
    multi_scale_features, world_points, depths, poses,
    ...
    """
    # (B, V, H, W, 3)
    # print('backprojector_dataloader world_points:', world_points.shape) # torch.Size([2, 5, 336, 336, 3])
    # print('world_points:', world_points[0][0].shape, len(world_points), len(world_points[0])) # torch.Size([2, 5, 20, 336, 336, 3])
    if world_points.dim() == 6:
        if world_points.shape[0] == 1:
            new_xyz = world_points.squeeze(0)
        elif world_points.shape[1] == 1:
            new_xyz = world_points.squeeze(1) 
        else:
            new_xyz =  world_points.flatten(0,1)  # This is B x V x H x W x 3: ([2, 5, 20, 336, 336, 3])
    else:
        new_xyz = world_points
    # if type(world_points) == list:
    #     flat_list = [x for sublist in world_points for x in sublist]
    #     new_xyz = torch.stack(flat_list, dim=0)
    # else: 
    #     raise NotImplementedError
    
    # new_xyz                 
    # new_xyz = world_points #[0] #backproject_depth(
    # print('new_xyz:', new_xyz.shape) # torch.Size([5, 20, 336, 336, 3])
        # depths, poses, intrinsics)
    # to_save = new_xyz.squeeze(0).reshape(-1, 3).to(torch.float32).cpu().numpy()
    # print('to_save:', to_save.shape) # (282240, 3)  
    # import open3d as o3d
    # pcd = o3d.geometry.PointCloud()
    # pcd.points = o3d.utility.Vector3dVector(to_save)
    # o3d.io.write_point_cloud("/local1/whu/tmp/data_check_in_llava3d.ply", pcd)
    ...
    # print('multi_scale_xyz[0]', multi_scale_xyz[0].shape) # torch.Size([20, 24, 24, 3])
