    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            ...
            if 'image' in sources[0]:
                ...
                ############ WENBO CHANGE: HARD CODE 
                elif isinstance(video_file, list):
                    # raise NotImplementedError
                    videos = [os.path.join(video_folder, v_file) for v_file in video_file]
                else:
                    raise NotImplementedError
                video_dicts = []
                for video in videos:
            ...
            labels = data_dict["labels"][0] if isinstance(i, int) else data_dict["labels"]
            non_ignored = (labels != IGNORE_INDEX).sum().item()
            ...
            if 'image' in self.list_data_dict[i] or 'video' in self.list_data_dict[i]:
                data_dict['image'] = images      #image  # (3, H, W) or (V, 3, H, W)
            ...
            if 'video' in self.list_data_dict[i]:
                ...
                world_points = []
                for video_dict in video_dicts:
                    #print (video_dict["world_points"].shape)
                    # depth.append(video_dict['depth_images'])
                    # pose.append(video_dict['poses'])
                    # intrinsic.append(video_dict['intrinsic'])
                    world_points.append(video_dict['world_points'])
                # data_dict['depth'] = torch.stack(depth)
                # data_dict['pose'] = torch.stack(pose)
                # data_dict['intrinsic'] = torch.stack(intrinsic)
                world_points = [torch.nan_to_num(wp, nan=0.0, posinf=0.0, neginf=0.0) for wp in world_points]

                min_len = min(wp.shape[0] for wp in world_points)
                data_dict['world_points'] = torch.stack([wp[:min_len] for wp in world_points])
                data_dict['image'] = [im[:min_len] for im in data_dict['image']]
                # data_dict['world_points'] =  torch.stack(world_points) 
            return data_dict
        except:
            return self.__getitem__(i-1)
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in instances[0]:
            ...
            #if torch.isnan(batch['images'][0]).any() or torch.isinf(batch['images'][0]).any():
            #    print(f"WARNING: images has NaN: {torch.isnan(batch['images']).sum().item()}, Inf: {torch.isinf(batch['images']).sum().item()}")

        instances = [instance for instance in instances if 'world_points' in instance]  # find the video data
        if len(instances) > 0:  # exist video data in the batch data
            ...
            world_points = [instance['world_points'] for instance in instances]
            ...
            # batch['depths'] =depths  #torch.stack(depths)  # (mini_b_2, V, H, W)
            # batch['poses'] =poses #torch.stack(poses)  # (mini_b_2, V, 4, 4)
            # batch['intrinsics'] = intrinsics #torch.stack(intrinsics)  # (mini_b_2, 4, 4)
            batch['world_points'] = torch.stack(world_points)   
            
            if torch.isnan(batch['world_points']).any() or torch.isinf(batch['world_points']).any():
                print(f"WARNING: world_points has NaN: {torch.isnan(batch['world_points']).sum().item()}, Inf: {torch.isinf(batch['world_points']).sum().item()}")
...
def train(attn_implementation=None):
    ...
    print(f"Train dataloader length (iterations per epoch): {len(trainer.get_train_dataloader())}")
    print(f"Total training steps: {trainer.args.num_train_epochs * len(trainer.get_train_dataloader())}")
