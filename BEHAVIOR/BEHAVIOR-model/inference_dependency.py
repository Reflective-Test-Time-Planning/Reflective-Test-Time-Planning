...
POINTCLOUD_DIR = "room_pointclouds_dependency"
...
def select_best_action_by_internal_reflection(
    ...
    scores = [ar['numeric_score'] for ar in action_reflections]
    max_score = max(scores)

    # Check if all scores are the same
    all_same_score = all(s == scores[0] for s in scores)

    if all_same_score:
        # Tie-break using candidate_count[action]
        best_action_data = max(
            action_reflections,
            key=lambda x: candidate_count.get(x['action'], 0)
        )
    else:
        # Normal highest-score selection
        best_action_data = max(
            action_reflections,
            key=lambda x: x['numeric_score']
        )
...
def generate_multiple_actions_from_model(
    ...
    while len(unique_actions) < num_actions and attempts < max_attempts:
        ...
        output = run_inference(
            ...
            world_points=video_data['world_points'],
            max_new_tokens=512,
            temperature=1.8,  # Higher temperature for diversity
        ...
        # Validation logic (adjust based on your task)
    ...
    return unique_actions, candidate_count
...
def build_retro_reflection_prompt(
    ...
    for action, data in room_actions_reflections.items():
...
def perform_rl_training(
    ...
    if len(negative_examples) - len(positive_examples) > 1 and len(positive_examples) > 0:
...
class BehaviorExecutor:
    ...
    def __init__(self, scene_name: str, total_rooms: List[str], scene_dir_name: str,
        ...
        # Replace this line:
        # With:
        self.retro_buffer = {}  # action -> {action, room, reflection, score}
        self.retro_reflected_actions = set()  # Track which actions have been retro-reflected
        ...
        if scene_config_file and os.path.exists(scene_config_file):
            ...
            parsed_ans = self.scene_config.get('parsed_ans', {})
            self.new_objects = parsed_ans.get('new objects', [])
    ...
    def execute_put_down(self, object_name: str, room_name: str = None) -> tuple:
        ...
        if not object_name.lower() in obj.name.lower():
            print ("Robot not holding the specific object")
            return False, "Robot not holding the specific object", None
...
def main():
    ...
    parsed_ans = scene_config.get('parsed_ans', {})
    task = parsed_ans.get('Task', 'Complete the storage task')
    total_rooms = parsed_ans.get('Rooms', [])

    target_room = total_rooms[0]

    for room in total_rooms:
        if room.lower() in task.lower():
            target_room = room

    # Extract target objects in order from task description
    new_objects = parsed_ans.get('new objects', [])
    
    target_objects = []
    for obj in new_objects:
        obj_name = obj['category']
        # Find first occurrence of each object name in task
        idx = task.lower().find(obj_name.lower())
        if idx != -1:
            target_objects.append((idx, obj_name))

    # Sort by appearance order in task
    target_objects.sort(key=lambda x: x[0])
    target_objects = [obj_name for idx, obj_name in target_objects]

    if not len(target_objects) == len(new_objects):
        os._exit(404)
    
    print ("......................................target.......................................")
    print (target_room, target_objects)


    # Extract target object (most liked item) for task completion check
    new_objects = parsed_ans.get('new objects', [])
    ...
    # Add:
    exit_rooms = []
    tts = True  # Flag to control when to use action selection
    ...
    for iteration in range(args.max_iterations):
        ...
        if args.experiment_type in ['vanilla', 'wo-ria']:
            ...
            num_actions, temp = 4, 2.5


        if "exit" in previous_action.lower():
            print(f"\n{'='*60}")
            print(f" ALL ROOMS EXPLORED - Using Action Selection with Internal Reflection")
            print(f"{'='*60}")
            ...
            candidate_actions, candidate_count = generate_multiple_actions_from_model(
            ...
            tts = False  # Only do this once after all rooms explored

        else: 
             # ORIGINAL BEHAVIOR: Generate single action when not all rooms explored
            print(f"\n--- Generating Action (Rooms explored: {len(explored_rooms)}/{len(total_rooms)}) ---")
            ...
            for attempt in range(max_action_attempts):
                ...
                if previous_action and "PICK" in previous_action.upper():
                    ...
                    parts = previous_action.split(" in ")[0].strip()  # Remove " in room" part
        ...
        if parsed_action["action_type"] == "EXIT" and success and (args.experiment_type not in ['vanilla', 'wo-roa']):                   
            ...
            for action_data in buffer_actions:
                ...
                print(f"\n--- Retro-Reflecting on Buffer Action #{action_idx}: {action_text} (from {action_room}) ---")
            ...
            if args.experiment_type != 'wo-action':
                print(f"\n{'='*70}")
                print(f" ALL ROOMS EXPLORED - TRIGGERING RL TRAINING")
                print(f"{'='*70}")
                ...
                print(f"\n  Not all rooms explored yet ({len(exit_rooms)}/{len(total_rooms)}), skipping RL training")
            ...
            if len(retro_reflections) > 0:
                ...
                # Step 2: Identify test actions (trained and untrained)
                ...
                trained_goto_actions = [ex['action'] for ex in goto_examples]
                trained_pickup_actions = [ex['action'] for ex in pickup_examples]
                ...
                if len(available_rooms) > 0:
                    ...
                    # Generate 2 additional random actions for comparison only (not trained)
                    random_goto = f"GO TO {available_rooms[1]}" if len(available_rooms) > 1 else "GO TO bedroom_0"
                    if exit_nums > 1:
                        random_pickup = pickup_examples[-1]['action'].replace(pickup_examples[-1]['action'].split(" ")[-1], available_rooms[1]) if len(available_rooms) > 1 else pickup_examples[-1]['action'].replace(pickup_examples[-1]['action'].split(" ")[-1], "bedroom_0") 



                # Step 3: Test BEFORE training (get baseline outputs for ALL 6 actions)
                print(f"\n{'='*70}")
                ...
                # # Add all trained GO TO actions
                # for idx, action in enumerate(trained_goto_actions):
                #     test_actions.append((action, f"Trained GO TO #{idx+1} (retro GT)"))
                
                # # Add all trained PICK UP actions  
                # for idx, action in enumerate(trained_pickup_actions):
                #     test_actions.append((action, f"Trained PICK UP #{idx+1} (retro GT)"))
                ...
                if len(available_rooms):
                    ...
                    # # Add random actions
                    # test_actions.append((random_goto, "Random GO TO (not trained)"))

                    # if exit_nums > 1:
                    #     test_actions.append((random_pickup, "Random PICK UP (not trained)"))
                ...
                for test_action, label in test_actions:
                    if test_action:
                        ...
                        print(f"\n{label}:")
                        print(f"  Raw output: {result['full_output']}")
                        print(f"  Formatted: {result['formatted_internal_reflection']}")

                        torch.cuda.empty_cache()

                #  Extra cleanup after all before tests
                print("\n Cleaning up before-training tests...")
                torch.cuda.empty_cache()
                ...
                for result in before_results:
                    ...
                    if "original GT" in result['label']:
                        ...
                        untrained_example = {
                            ...
                            'action_type': action_type,
                            'prompt': prompt_text,
                            'ground_truth': ground_truth,  #  Use raw output directly
                            'video_paths': current_video_paths,
                            'action_idx': -1,
                ...
                # Step 4.5: SHUFFLE training examples
                print(f"\n Shuffling training examples...")
                random.shuffle(training_examples)

                # Step 5: Perform Test-Time Training on TTT MODEL
                if args.experiment_type != 'wo-value':
        ...
        # Task is complete if: all rooms explored AND picked up correct object (target_object)
        
        # Check if the PICK UP action in this iteration used correct object
        
        
        
        print ("...........................................................................................")
        print (current_room == target_room, "put down" in parsed_action["raw"].lower(), target_objects[0].lower() in parsed_action["raw"].lower(), success)

        if current_room == target_room and "put down" in parsed_action["raw"].lower() and target_objects[0].lower() in parsed_action["raw"].lower() and success:
            print ("!!!!!!!!!!!!!!!!!!!!!!!!!successfully put down target object!!!!!!!!!!!!!!!!!!!!!!!!!!")
            target_objects.pop(0)
        ...
        if task_completed_now:
            ...
            # Save results immediately and exit to avoid cleanup crash
            if args.output_file:
                results = {
                    "task": task,
                    "scene": scene_name,
                ...
                os._exit(42)  # Hard exit - bypass Python cleanup to avoid segfault
    ...
    # Save detailed retro-reflection data to tmp.json
    #         "task": task,
    #         "scene": scene_name,
    #         "total_rooms": total_rooms,
    #         "retro_reflections": retro_reflection_data
    #     }
        
    
    # Save results
    if args.output_file:
        # Check task completion for final status
        all_rooms_explored = set(exit_rooms) == set(total_rooms)
        ...
        results = {
            ...
            "total_rooms": total_rooms,
            "iterations": iteration_results,
            # "retro_reflections": retro_reflections,
            "action_history": executor.action_history,
        ...
        if task_completed:
            print(f"\n TASK COMPLETED SUCCESSFULLY!")
            os._exit(42)  # Hard exit - bypass cleanup
        else:
            print(f"\n TASK INCOMPLETE")
            os._exit(43)  # Hard exit - bypass cleanup
