...
POINTCLOUD_DIR = "room_pointclouds_hybrid"
...
def main():
    ...
    target_objects = parsed_ans.get('object_order', [])
    ...
    for iteration in range(args.max_iterations):
        ...
        if parsed_action["action_type"] == "EXIT" and success and (args.experiment_type not in ['vanilla', 'wo-roa']):                              
            ...
            # === RL TRAINING SECTION (ON EXIT WITH ALL ROOMS EXPLORED) ===
            ...
            if len(retro_reflections) > 0:
                ...
                for idx, ex in enumerate(training_examples):
                    print(f"\n{'='*70}")
                    print(f"Training Example {idx+1}/{len(training_examples)}")
                    print(f"{'='*70}")
                    print(f"Action: {ex['action']}")
                    print(f"Action Type: {ex['action_type']}")
                    print(f"Training Type: {ex['training_type']}")
                    print(f"\n--- INPUT PROMPT ---")
                    print(ex['prompt'])
                    print(f"\n--- GROUND TRUTH OUTPUT ---")
                    print(ex['ground_truth'])
                    print(f"\n--- VIDEO PATHS ---")
                    for vp in ex['video_paths']:
                        print(f"  {vp}")
                    print(f"{'='*70}")
                ...
                # Step 6: Test AFTER training (all 6 actions)
                print(f"\n{'='*70}")
                print(f" TESTING AFTER TRAINING (6 actions)")
                print(f"{'='*70}")
                ...
                for test_action, label in test_actions:
                    if test_action:
                        ...
                        print(f"\n{label}:")
                        print(f"  {result['formatted_internal_reflection']}")
    ...
    if args.output_file:
        ...
        # Using os._exit() to avoid Python cleanup which triggers OmniGibson segfault
        if task_completed:
            print(f"\n TASK COMPLETED SUCCESSFULLY!")
